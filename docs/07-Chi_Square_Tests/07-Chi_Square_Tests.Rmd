...
- no opinion of offshore drilling? (*categorical*)
- IMDB.com rating (*numerical*)
- income (*numerical*)
]
...
- college graduate? (two levels; *categorical*)
- movie genre (two levels; *categorical*)
- city (two levels; *categorical*)
]
...
Zodiac <- read_csv("~/Dropbox/Teaching/03-Simmons Courses/STAT227-Statistical Design and Analysis [ONLINE]/Data/Zodiac.csv")
```
...
- As *n* (and hence, the *df*) increases, the degree of skewness in the $\chi^{2}$ distribution changes. 
...
From STAT 338/339: The **expected value** (i.e., the mean) of the $\chi^{2}$ distribution is the *df*!
...
```{r}
bite_tab = sort(tally( ~ breed, data = nyc_bites), decreasing = TRUE)
bite_tab
```
...
nyc_bites_200 = nyc_bites %>%
  filter(breed %in% names(bite_tab[1:8]))
```
...
gss <- readr::read_csv("~/Dropbox/Teaching/03-Simmons Courses/STAT227-Statistical Design and Analysis [ONLINE]/Data/gss.csv")
```
...
# Permutation Chi-Square Goodness of Fit Test (with R)
...
Up until now, we have been conducting chi-square tests by comparing our **observed statistic** to a *theoretical distribution* in a world where the null hypothesis is true. 

Permutation tests, as they did when comparing one or two groups, allow us to **simulate** the null hypothesis distribution through permutations. 

> *If we live in world where variables are totally unrelated, the ties between variables are arbitrary, so they might just as well have been shuffled.* - Andrew Bray
